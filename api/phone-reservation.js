...
export default async function handler(request, response) {
  ...
  try {
    ...
    const carrier = clean(b.carrier,30), currentModel = clean(b.currentModel,60), model = clean(b.model,60), color = clean(b.color,40), capacity = clean(b.capacity,30);
    if (!seller || !["신규","번호이동","기기변경"].includes(type) || !name || !/^\d{8}$/.test(birth) || !/^01\d{8,9}$/.test(phone) || !currentModel || !model || !color || !capacity || (type === "번호이동" && !carrier)) return response.status(400).json({success:false,message:"입력 내용을 확인해주세요."});
    ...
    lines.push(`📲 <b>현재 기종</b>  ${esc(currentModel)}`,`⭐ <b>예약 모델</b>  ${esc(model)}`,`🎨 <b>색상</b>  ${esc(color)}`,`💾 <b>용량</b>  ${esc(capacity)}`,`🕒 <b>접수시간</b>  ${esc(time)}`);
